...
wait_for_client() {
  ...
  for i in $(seq 1 "${max_retries}"); do
    if authenticated_request "http://${INTEGTEST_HOST}/api/email/register/developer${client}.lokole.ca" >"${out_dir}/register${client}.json"; then
...
authenticated_request \
  "http://${INTEGTEST_HOST}/api/email/register/" \
...
authenticated_request \
  "http://${INTEGTEST_HOST}/api/email/register/developer3.lokole.ca" \
  -X DELETE
